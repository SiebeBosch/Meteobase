...
Module MBSTOCHASTEN
  ...
  Dim StochData As New HYDROC01.clsMBStochData(Setup)
  Sub Main()
    ...
    '----------------------------------------------------------------------------------------------------------------------------
    If RunOnServer Then
      StochData.DownloadURL = "http://62.148.170.210/meteobase/downloads/"                                                 'laptop en server
      StochData.DownloadDIR = "c:\Program Files (x86)\PostgreSQL\EnterpriseDB-Apache\Php\apache\www\meteobase\downloads\"  'server
      StochData.FilesDir = "c:\Program Files (x86)\PostgreSQL\EnterpriseDB-Apache\Php\apache\www\meteobase\downloads\fixed\"  'server
    Else
      StochData.DownloadURL = "http://62.148.170.210/meteobase/downloads/"                                                 'laptop en server
    ...
    ElseIf My.Application.CommandLineArgs.Count <> 12 Then
    ...
    Else
      ...
      StochData.SessionID = My.Application.CommandLineArgs(8)
      StochData.OrderNum = My.Application.CommandLineArgs(9)
      StochData.Naam = Setup.GeneralFunctions.RemoveBoundingQuotes(My.Application.CommandLineArgs(10))
      StochData.MailAdres = Setup.GeneralFunctions.RemoveBoundingQuotes(My.Application.CommandLineArgs(11))
    End If
