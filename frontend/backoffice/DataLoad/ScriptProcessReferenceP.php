<html>
  <head>
    <title>Process Referentie H+</title>
  </head>
...
<?php 
    ...
    // **
    // ** 2012-04-10
    // ** ScriptProcessReferentieP.php       v-67
...
//      Foldername is hard-coded :                                      // *
	...
	$FileHardCoded = "Regio_HP_1906-2010.txt";                       // *
	...
	for ($b = 1 ; $b <= ($Batches +1);$b++)
	{
		...
		for ($i = ((($BatchCounter-1)  * $BatchGrootte) + $StartValue) ; $i < (($BatchCounter  * $BatchGrootte) + $StartValue) ;$i++)
		{
			if ($i <= $StopValue)
			{
				...
				if (check_record($ExtraH , $Date , $Hour , $Wetness) == "FALSE")
				{
				...
				{
					add_record($ExtraH , $Date , $Hour , $Wetness, $RecordCounter);
...
//-------------------------------------------------------------------
function add_record( $pExtraH , $pDate , $pHour , $pWetness,$pRecordCounter)
{
// This function adds a new record to the Wetnesses-array :
	global $Wetnesses ;

	$NewIndex = $pRecordCounter ;
	$NewRecord = Array($pExtraH , $pDate , $pHour , $pWetness);
//echo $pExtraH . "--" . $pDate . "++" . $pHour . "--" . $pWetness  . "++" .  "<br>";
...
// This function dumps the Reference-array into the database :
	...
	$Key = assemble_keys($Keys,'data.regionhpraw');
    ...
    // Access the table :
    	$sQuery = "SELECT * FROM data.regionhpraw" ;   
