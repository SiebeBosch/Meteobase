...
if(isset($_POST['userFullNameFeedback'])) {
	...
	//$cmd = '"C:/Program Files (x86)/Hydroconsult/WIWBFEEDBACK/WIWBFEEDBACK.exe" "'. $naam . '" "' . $mail . '" "' . $feedbackType . '" "' . $feedbackMessage . '" ';
	//$cmd = 'start /B cmd /C "' . $cmd . ' >NUL 2>NUL"';
	//pclose(popen($cmd, 'r'));
	$_SESSION["feedbackMsg"]='Bedankt voor je bericht.';
	include('aanmeld01.php');
} else {
	$_SESSION['errorMsg'] = "Is een fout opgetreden. Probeer het later opnieuw";
	include('aanmeld01.php');
}
